...
void DepthLapdepth::image_callback(const sensor_msgs::msg::Image::ConstSharedPtr msg)
{
  ...
  if (!depth_result.mat_out.empty()) {
    ...
    cv::minMaxLoc(depth_result.mat_out, &depth_min, &depth_max);
    depth_result.mat_out.convertTo(image_result, CV_8UC1, 255. / (depth_max - depth_min), (-255. * depth_min) / (depth_max - depth_min));
    cv::applyColorMap(image_result, image_result, cv::COLORMAP_MAGMA);
